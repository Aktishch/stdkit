...
import { DataForm, DialogRemoveProps, TSXComponent } from '@utils'
import { SubmitHandler, useForm } from 'react-hook-form'
import { twMerge } from 'tailwind-merge'

export const DialogRemove = ({ className, open, onClose, data }: DialogRemoveProps): TSXComponent => {
  ...
  return (
    <Dialog className={style} open={open} onClose={onClose}>
      <div className="items-center justify-center px-4 text-center py-7 card-content">
        ...
        <h2 className="mb-2 font-medium text-lg/normal">Вы точно хотите удалить?</h2>
        <p className="mb-6 font-normal text-sm/normal opacity-60">(Вся информация будет удалена из базы)</p>
        <form className="grid w-full grid-cols-2 gap-4" onSubmit={handleSubmit(submitHandler)}>
